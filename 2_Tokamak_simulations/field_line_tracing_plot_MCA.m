...
plot3(upper_DIV_coil_data(1).coords_coil(:,1),upper_DIV_coil_data(1).coords_coil(:,2)...
    ,upper_DIV_coil_data(1).coords_coil(:,3),'K','linewidth',2)
plot3(lower_DIV_coil_data(1).coords_coil(:,1),upper_DIV_coil_data(1).coords_coil(:,2)...
    ,lower_DIV_coil_data(1).coords_coil(:,3),'K','linewidth',2)

xlabel('x (m)')
ylabel('y (m)')
zlabel('z (m)')
%title('ST25-D field line')
legend([hb,ht,hp],'Magnetic field lines','Toroidal field coils','Poloidal field coils','Location','best')
view(-14,4)
 view(30,30)
% view(0,0)
ax=gca;
...
legend([hp,hplasm,hpc],'Poincaré points','Plasma boundary','Poloidal field coils')
